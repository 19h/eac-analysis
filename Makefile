...
MBA_PROBLEM_INVENTORY_TSV := $(PRIMARY_DIR)/vm_mba_problem_inventory.tsv
MBA_OBSERVATION_ROWS_TSV := $(PRIMARY_DIR)/vm_mba_observation_rows.tsv
MBA_FORMULA_ATOMS_TSV := $(PRIMARY_DIR)/vm_mba_formula_atoms.tsv
MBA_TRACE_INPUTS_TSV := $(PRIMARY_DIR)/vm_mba_trace_inputs.tsv
MBA_SOLUTION_CORPUS_MD := $(PRIMARY_DIR)/vm_mba_solution_corpus.md
MBA_STATE_REDUCTIONS_TSV := $(PRIMARY_DIR)/vm_mba_state_formula_reductions.tsv
MBA_STATE_REDUCER_HARNESS_C := $(PRIMARY_DIR)/vm_mba_state_formula_reducer_harness.c
MBA_STATE_REDUCER_HARNESS_SO := $(PRIMARY_DIR)/vm_mba_state_formula_reducer_harness.so
MBA_STATE_REDUCTIONS_MD := $(PRIMARY_DIR)/vm_mba_state_formula_reductions.md
...
STATIC_TRANSFER_EXPR_FULL_PYTHON_TSV := $(PRIMARY_DIR)/vm_static_transfer_expr_full_python.tsv
MBA_DISPATCH_COMPILER_REDUCTIONS_TSV := $(PRIMARY_DIR)/vm_mba_dispatch_formula_compiler_reductions.tsv
MBA_DISPATCH_COMPILER_REDUCTIONS_BY_ENTRY_TSV := $(PRIMARY_DIR)/vm_mba_dispatch_formula_compiler_reductions_by_entry.tsv
...
DISPATCH_SECOND_STAGE_BINDING_ROWS_TSV := $(PRIMARY_DIR)/vm_dispatch_second_stage_binding_rows.tsv
DISPATCH_SECOND_STAGE_BINDING_SUMMARY_TSV := $(PRIMARY_DIR)/vm_dispatch_second_stage_binding_summary.tsv
DISPATCH_SECOND_STAGE_BINDING_C := $(PRIMARY_DIR)/vm_dispatch_second_stage_bindings.c
DISPATCH_SECOND_STAGE_BINDING_MD := $(PRIMARY_DIR)/vm_dispatch_second_stage_bindings.md
...
X_BEHAVIOR_MODEL_MD := $(PRIMARY_DIR)/vm_x_behavior_model.md
...
PROGRAM_MBA_010_IR_ROWS_TSV := $(PROGRAM_MBA_010_PREFIX)_ir_rows.tsv
PROGRAM_MBA_010_REQUIREMENTS_TSV := $(PROGRAM_MBA_010_PREFIX)_requirements.tsv
PROGRAM_MBA_010_MISSING_TSV := $(PROGRAM_MBA_010_PREFIX)_missing.tsv
PROGRAM_MBA_010_TRACE_TARGETS_TSV := $(PROGRAM_MBA_010_PREFIX)_trace_targets.tsv
...
UNCOVERED_EXECUTABLE_GAPS_C := $(PRIMARY_DIR)/vm_uncovered_executable_gaps.c
UNCOVERED_EXECUTABLE_GAPS_TSV := $(PRIMARY_DIR)/vm_uncovered_executable_gaps.tsv
UNCOVERED_EXECUTABLE_GAPS_MD := $(PRIMARY_DIR)/vm_uncovered_executable_gaps.md
...
.PHONY: uncovered-executable-gaps

$(UNCOVERED_EXECUTABLE_GAPS_C): vm_uncovered_executable_gaps.py eac.elf $(NATIVE_EXECUTABLE_COVERAGE_AUDIT_TSV)
	python3 vm_uncovered_executable_gaps.py --elf eac.elf --coverage $(NATIVE_EXECUTABLE_COVERAGE_AUDIT_TSV) --format c > $@

$(UNCOVERED_EXECUTABLE_GAPS_TSV): vm_uncovered_executable_gaps.py eac.elf $(NATIVE_EXECUTABLE_COVERAGE_AUDIT_TSV)
	python3 vm_uncovered_executable_gaps.py --elf eac.elf --coverage $(NATIVE_EXECUTABLE_COVERAGE_AUDIT_TSV) --format tsv > $@

$(UNCOVERED_EXECUTABLE_GAPS_MD): vm_uncovered_executable_gaps.py eac.elf $(NATIVE_EXECUTABLE_COVERAGE_AUDIT_TSV)
	python3 vm_uncovered_executable_gaps.py --elf eac.elf --coverage $(NATIVE_EXECUTABLE_COVERAGE_AUDIT_TSV) --format markdown > $@

uncovered-executable-gaps: $(UNCOVERED_EXECUTABLE_GAPS_C) $(UNCOVERED_EXECUTABLE_GAPS_TSV) $(UNCOVERED_EXECUTABLE_GAPS_MD)
...
program-mba-all:
	python3 vm_program_mba_all_reduce.py
...
.PHONY: program-behavior-dossiers program-behavior-dossiers-audit
$(PROGRAM_BEHAVIOR_DOSSIERS_TSV) $(PROGRAM_BEHAVIOR_DOSSIERS_MD) &: vm_program_behavior_dossiers.py $(BEHAVIOR_INVENTORY_TSV) $(PROGRAM_BEHAVIOR_HYPOTHESES_TSV) $(PROGRAM_CONTROL_GRAPH_SUMMARY_TSV) $(X_PROGRAM_FIRST_SEEN_TSV) $(STRING_ROLE_ANNOTATIONS_TSV) $(NATIVE_SIDE_EFFECT_VM_REFS_TSV) $(NATIVE_SIDE_EFFECT_PROGRAMS_TSV) $(PROGRAM_OPCODE_PSEUDOCODE_TSV) $(PROGRAM_OPCODE_PSEUDOCODE_MANIFEST)
	python3 vm_program_behavior_dossiers.py --root $(PRIMARY_DIR)

program-behavior-dossiers: $(PROGRAM_BEHAVIOR_DOSSIERS_TSV) $(PROGRAM_BEHAVIOR_DOSSIERS_MD)
...
.PHONY: mba-solution-corpus mba-solution-corpus-audit
$(MBA_PROBLEM_INVENTORY_TSV) $(MBA_OBSERVATION_ROWS_TSV) $(MBA_FORMULA_ATOMS_TSV) $(MBA_TRACE_INPUTS_TSV) $(MBA_SOLUTION_CORPUS_MD) &: vm_mba_solution_corpus.py $(SEMANTIC_OPCODE_CATALOG_TSV) $(PROGRAM_READABLE_OPS_TSV) $(NATIVE_SIDE_EFFECT_VM_REFS_TSV) $(PRIMARY_DIR)/vm_instruction_trace.tsv $(PRIMARY_DIR)/vm_instruction_trace_filefill_hiddenfill_frontierfill_footprintfill.tsv
	python3 vm_mba_solution_corpus.py --root $(PRIMARY_DIR)

mba-solution-corpus: $(MBA_PROBLEM_INVENTORY_TSV) $(MBA_OBSERVATION_ROWS_TSV) $(MBA_FORMULA_ATOMS_TSV) $(MBA_TRACE_INPUTS_TSV) $(MBA_SOLUTION_CORPUS_MD)

mba-solution-corpus-audit: $(MBA_PROBLEM_INVENTORY_TSV) $(MBA_OBSERVATION_ROWS_TSV) $(MBA_FORMULA_ATOMS_TSV) $(MBA_TRACE_INPUTS_TSV) $(MBA_SOLUTION_CORPUS_MD)
	python3 vm_mba_solution_corpus_audit.py --root $(PRIMARY_DIR)

.PHONY: mba-state-formula-reducer mba-state-formula-reducer-audit
$(MBA_STATE_REDUCTIONS_TSV) $(MBA_STATE_REDUCER_HARNESS_C) $(MBA_STATE_REDUCER_HARNESS_SO) $(MBA_STATE_REDUCTIONS_MD) &: vm_mba_state_formula_reducer.py $(MBA_PROBLEM_INVENTORY_TSV)
	python3 vm_mba_state_formula_reducer.py --root $(PRIMARY_DIR)

mba-state-formula-reducer: $(MBA_STATE_REDUCTIONS_TSV) $(MBA_STATE_REDUCER_HARNESS_C) $(MBA_STATE_REDUCER_HARNESS_SO) $(MBA_STATE_REDUCTIONS_MD)

mba-state-formula-reducer-audit: $(MBA_STATE_REDUCTIONS_TSV) $(MBA_STATE_REDUCER_HARNESS_C) $(MBA_STATE_REDUCER_HARNESS_SO) $(MBA_STATE_REDUCTIONS_MD)
	python3 vm_mba_state_formula_reducer_audit.py --root $(PRIMARY_DIR)

.PHONY: mba-state-formula-prover mba-state-formula-prover-audit
$(MBA_STATE_PROOFS_TSV) $(MBA_STATE_PROOFS_MD) &: vm_mba_state_formula_prover.py $(MBA_STATE_REDUCTIONS_TSV)
	python3 vm_mba_state_formula_prover.py --root $(PRIMARY_DIR)

mba-state-formula-prover: $(MBA_STATE_PROOFS_TSV) $(MBA_STATE_PROOFS_MD)
...
x-behavior-model: $(X_RUNTIME_EVENT_SEQUENCE_TSV) $(X_BEHAVIOR_PHASE_SUMMARY_TSV) $(X_PROGRAM_PHASE_OVERLAY_TSV) $(X_BEHAVIOR_MODEL_MD)

x-behavior-model-audit: $(X_RUNTIME_EVENT_SEQUENCE_TSV) $(X_BEHAVIOR_PHASE_SUMMARY_TSV) $(X_PROGRAM_PHASE_OVERLAY_TSV) $(X_BEHAVIOR_MODEL_MD)
...
native-gap-retdec-checkpoint: $(NATIVE_RETDEC_GAP_QUEUE_TSV)
	python3 vm_native_gap_retdec_checkpoint.py
native-gap-retdec-autopilot: $(NATIVE_RETDEC_GAP_QUEUE_TSV)
	python3 vm_native_gap_retdec_autopilot.py
.PHONY: executable-gap-retdec-probe
executable-gap-retdec-probe: $(NATIVE_EXECUTABLE_COVERAGE_AUDIT_TSV)
	python3 vm_executable_gap_retdec_probe.py
...
reconstruction-autopilot: $(NATIVE_EXECUTABLE_COVERAGE_AUDIT_TSV)
	python3 vm_executable_gap_retdec_autopilot.py $(RECONSTRUCTION_AUTOPILOT_ARGS)

.PHONY: reconstruction-unattended
reconstruction-unattended: $(NATIVE_EXECUTABLE_COVERAGE_AUDIT_TSV)
	python3 vm_reconstruction_unattended.py $(RECONSTRUCTION_UNATTENDED_ARGS)

.PHONY: reconstruction-status
...
pseudocode-syntax-check: $(PSEUDOCODE_TOP_C) $(PROGRAM_PSEUDOCODE_TOP_C) $(PROGRAM_PSEUDOCODE_FULL_C) $(PROGRAM_DECOMPILED_FULL_C) $(HANDLERS_PSEUDOCODE_C) $(PATH_HANDLERS_PSEUDOCODE_C) $(PATH_HANDLERS_FRONTIER_PSEUDOCODE_C) $(PATH_FRONTIER_SLOT_AUDIT_C) $(NATIVE_EXECUTABLE_COVERAGE_AUDIT_C) $(NATIVE_FUNCTION_INVENTORY_C) $(NATIVE_RETDEC_GAP_QUEUE_C) $(NATIVE_GAP_RETDEC_BATCH_CS) $(SOURCE_BUNDLE_C) $(ALL_EVIDENCE_BUNDLE_C) $(NATIVE_RET_PATCH_TARGETS_C) $(NATIVE_RET_PATCH_EPILOGUES_RETDEC_C) $(NATIVE_RET_PATCH_SOURCE278_RETDEC_C) $(RET_PATCH_FOLLOWUPS_C) $(RET_PATCH_FOLLOWUP_RETDEC_C) $(NATIVE_OBFUSCATED_ISLANDS_C) $(NATIVE_OBFUSCATED_SECOND_STAGE_C) $(NATIVE_OBFUSCATED_SECOND_STAGE_DYNAMIC_C) $(NATIVE_OBFUSCATED_SECOND_STAGE_SLOT_PROOF_C) $(NATIVE_OBFUSCATED_SECOND_STAGE_STACK_SOURCE_C) $(NATIVE_OBFUSCATED_SECOND_STAGE_RBX_PROVENANCE_C) $(NATIVE_OBFUSCATED_SECOND_STAGE_MODEL_C) $(NATIVE_OBFUSCATED_CONTROL_MODEL_C) $(NATIVE_RET_PATCH_HIDDEN_BRIDGE_C) $(NATIVE_HANDLER_ENVIRONMENT_COVERAGE_C) $(CONFIG_COVERAGE_FRONTIER_C) $(BINARY_DATA_SECTIONS_C) $(STATIC_ONLY_HANDLER_QUEUE_C) $(STATIC_ONLY_TIER0_MODELS_C) $(STATIC_ONLY_TIER1_MODELS_C) $(STATIC_ONLY_TIER2_SPLIT_C) $(STATIC_ONLY_TIER3_SHARED_C) $(STATIC_ONLY_TIER4_CALLRET_C) $(STATIC_ONLY_TIER5_LARGE_C) $(TARGET_ONLY_HANDLER_RETDEC_C) $(UNOBSERVED_HANDLER_RETDEC_CS) $(WEAK_HANDLER_RETDEC_C) $(VALIDATED_HANDLER_RETDEC_CS) $(UNRESOLVED_FAMILY_C)
	...
	$(CC) -std=c11 -fsyntax-only -Wall -Wextra -Wno-unused-variable -Wno-unused-function -Wno-parentheses dumps/vmtail-wide-1m-w16/vm_program_decompiled_full.c

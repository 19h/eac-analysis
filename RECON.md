...
`vm_program_pseudocode_dump.py` then renders the recovered bytecode program in terms of that handler layer. `make pseudocode` emits `vm_program_pseudocode_top.c`, currently 26382 lines: 80 hottest VM basic blocks, 80 forward block declarations, 39 concrete block-to-block calls, and explicit handler-call/`next_entry`/`vm_ip` statements for selected recovered rows and synthetic terminals. `make pseudocode-full` emits `vm_program_pseudocode_full.c`, currently 296770 lines / 19 MiB: all 499 recovered VM basic blocks, 499 forward block declarations, 71667 rendered `op_entry_NNN(vm)` call occurrences, 71384 `next_entry` fallback assignments, 498 concrete block-to-block calls, 267 synthetic-successor calls, 214 executable synthetic-tail target loads, 21 transfer-probe evidence sites, 21 dynamic-stitch evidence sites / 22 event comments, 1 symbolic-successor audit site, 21 hidden-chain/next-hook probe sites / 22 comments, 20 residual/concrete/live/table/sample audit sites, 20 focused direct-trace audit comments, and 5 focused direct bridges promoted to concrete handler/block calls. The current full sketch has 15 explicit `vm_unresolved_synthetic_tail(vm, 0x...)` calls; the other 5 former residual starts (`0x18c467`, `0x24bd2a`, `0x31f2b1`, `0x33a6cc`, `0x34556c`) now execute the focused source handler and call the recovered destination block. The remaining 15 unresolved calls still carry disabled-by-default `VM_ENABLE_OBSERVED_REENTRY_BRIDGES` snippets because those are sequence-observed next-hook paths, not direct rows starting at the residual VM IP.

`make source-bundle` writes `vm_recovered_source_bundle.c`, currently 303796 lines / 20 MiB, combining all 360 `op_entry_NNN` handler bodies with the full 499-block bytecode program, 287 recovered synthetic transitions, 12 sampled-operand source-state sidecars, 3 sampled-operand-only handler caveats, 498 block-to-block calls, 15 explicit unresolved synthetic-successor calls, 214 synthetic-tail target-load expressions, 21 transfer-probe evidence sites, 21 dynamic-stitch evidence sites / 22 event comments, 1 symbolic-successor audit site / 1 candidate comment, 21 hidden-chain/next-hook probe sites / 22 comments, 20 residual/concrete/live/table/sample/focused audit sites, 1 exact hidden-chain reentry, 5 focused direct bridges, and 15 disabled observed reentry bridges. `make c-reconstruction-manifest` now regenerates the raw instruction trace, unique-instruction table, instruction lift, sampled recovery, file atlas, file-fill sidecars, IR, basic blocks, C sketches, syntax/object/link gates, coverage audit, synthetic-gap residual audit, concrete-state residual audit, live-context residual audit, table-read diagnostic, table-memory probe, sampled-control correlation, focused direct-trace audit, and manifest in dependency order, so stale trace rows no longer stop at the TSV boundary. The current manifest records 360 handler functions, 120 table-offset handler next-entry normalizations, 10 direct-slot handler normalizations, 560 path-specialized functions, 499 full program block functions, 15 explicit unresolved synthetic successor calls, 20 focused direct-trace audit rows, 5 focused direct bridge candidates/promotions, 15 disabled observed reentry bridge snippets, and `completion_status=not_complete`. This is the most readable and mechanically C-shaped program artifact so far: native handler arithmetic, bytecode block order, bytes, VM IPs, expected next entries, decoded control edges, recovered per-instruction `vm_ip` progress, recovered synthetic edges, synthetic suffix schemas, direct/boundary suffix target loads, operand-driven long-control loads, sampled-operand source effects, long-control overlap/prefix annotations, transfer-probe gap classifications, symbolic-slot successor audits, hidden-handler-chain reentry proof, dynamic next-hook validation, residual promotion-state audit, concrete-state replay audit, live-context replay audit, final table-read diagnostics, file-backed table-offset probes, sampled-control correlation, focused direct-trace bridges, and disabled observed next-hook bridges are available together in a source bundle that parses, codegens, links, smoke-runs, and has a reproducible manifest and remaining-gap report, with the separate `vm_handlers_pseudocode.c` and `vm_program_pseudocode_*.c` files kept as smaller views.

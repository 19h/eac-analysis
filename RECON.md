...
- `dumps/vmtail-wide-1m-w16/vm_native_retdec_gap_queue.c`, `.tsv`, and `.md`: ranked native function backlog for semantic C recovery. It records 2739 queued `.text` functions, 733554 function-boundary uncovered bytes, and a tier mix of 2411 small, 247 medium, 77 large, and 4 huge/noisy targets; the C sidecar keeps all 2739 rows in the all-evidence file.
...
The all-evidence bundle also carries `vm_native_retdec_gap_queue.c` as a renamed sidecar. This is the current strategic backlog for native semantic C recovery: 2739 `.text` functions still have bytes not covered by semantic RetDec/native C sidecars, totaling 733554 function-boundary uncovered bytes. The queue is deliberately tiered for fast targeted work: 2411 small single-function targets, 247 medium targets, 77 large targets, and 4 huge/noisy targets that should be split or modeled before broad RetDec.

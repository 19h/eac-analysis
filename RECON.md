...
- `dumps/vmtail-wide-1m-w16/vm_native_retdec_gap_queue.c`, `.tsv`, and `.md`: ranked native function backlog for semantic C recovery. It records 2955 queued `.text` functions, 810377 function-boundary uncovered bytes, and a tier mix of 2627 small, 247 medium, 77 large, and 4 huge/noisy targets; the C sidecar keeps all 2955 rows in the all-evidence file.
...
The all-evidence bundle also carries `vm_native_retdec_gap_queue.c` as a renamed sidecar. This is the current strategic backlog for native semantic C recovery: 2974 `.text` functions still have bytes not covered by semantic RetDec/native C sidecars, totaling 818462 function-boundary uncovered bytes. The queue is deliberately tiered for fast targeted work: 2646 small single-function targets, 247 medium targets, 77 large targets, and 4 huge/noisy targets that should be split or modeled before broad RetDec.
...
The normal C check and manifest refresh paths are no longer tied to phony pseudocode/reconstruction aggregate prerequisites. `pseudocode-syntax-check`, `pseudocode-object-check`, `pseudocode-link-check`, and `c-reconstruction-manifest` now depend on concrete generated C/artifact file targets where possible, so unchanged checks skip bytecode catalog, synthetic probe, coverage, and RetDec sidecar regeneration. The current manifest gate (`make -s c-reconstruction-manifest`) completed in 31.42 seconds after adding the native function-inventory, native RetDec gap queue, and native gap RetDec batch00/01/02/03/04/05 sidecar checks.

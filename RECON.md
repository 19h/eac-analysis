...
`vm_synthetic_tail_catalog.py` aggregates those compact tails across all synthetic groups. There are 116 exact unresolved tail variants covering 1570 events. By tail length: `0x6` has 66 variants / 628 events, `0x4` has 7 variants / 501 events, `0xb` has 14 variants / 230 events, `0x5` has 21 variants / 166 events, `0xc` has 5 variants / 18 events, `0x8` has 2 variants / 26 events, and `0x3` has 1 variant / 1 event. The hottest target-bearing forms are small endian target words plus the recurring `e801` marker: `a500e801` -> target 165 / 154 events, `e801ab00` -> target 171 / 105 events, `ab00e801` -> target 171 / 87 events, `e801a500` -> target 165 / 64 events, `e8016201` -> target 354 / 50 events, and `6201e801` -> target 354 / 40 events. The longer 11-byte tail family is dominated by sampled long-control leftovers with paired `3d01`/`9d01` motifs, while the target-only entry 50 tails remain the special `8000...` forms above. This makes the next semantic target a small set of tail mini-forms, not hundreds of independent byte gaps.

`vm_pseudocode_dump.py` is the first direct C-like reconstruction artifact. It renders the recovered VM IR as block functions over a `VMState { ip, state, flags, byte }`, translating `state0`, `flags0`, `u16_N`, `u32_N`, and byte operands into C-like expressions and emitting `next_entry = ...; vm->ip += ...;` for each recovered VM instruction. `make pseudocode` writes `dumps/vmtail-wide-1m-w16/vm_pseudocode_top.c`; the current file contains 60 hottest VM basic blocks / 12275 lines, with long blocks truncated to 32 recovered IR statements each. Its 21 selected synthetic terminal spans are now lifted into explicit source/target/delta comments plus `next_entry = ...` and `vm->ip += ...` statements; the only remaining `vm_unresolved_synthetic_tail` use is the default switch fallback. This is not polished source yet, but it is now a concrete C-shaped view of recovered behavior instead of only TSVs.

`vm_handler_pseudocode_dump.py` factors the VM ISA/operator layer out of the bytecode path. `make handler-pseudocode` emits `vm_handlers_pseudocode.c` for all 360 dispatch entries and `vm_handlers_hot_pseudocode.c` for the 80 hottest observed entries. The full file is 7082 lines and covers 166 `static_validated`, 155 `unobserved_static`, 13 `affine_dispatch_fallback`, 11 `partial`, 9 `sampled_only`, 3 `target_only`, and 3 `sampled_operand_lifted` handlers. Each `op_entry_NNN(VMState *vm)` carries native handler address, operand layout, native IP reads, state/flag expressions where safe, slot/IP updates where single-valued, observed top targets, validation status, and explicit comments for handlers not directly executed as source rows. When a handler has no source-row execution but the native tail site contains a constant VM-IP update, the dump now emits that static `vm->ip += ...` fallback; the current full file contains 157 such native-tail IP advances, including target-only entries 5, 50, and 80. This is the static/global C-like layer; the block pseudocode remains the dynamic bytecode path layer.

`vm_program_pseudocode_dump.py` then renders the recovered bytecode program in terms of that handler layer. `make pseudocode` emits `vm_program_pseudocode_top.c`, currently 13488 lines: 80 hottest VM basic blocks, 6182 `op_entry_NNN(vm)` calls, 154 referenced handler prototypes, and 29 recovered synthetic terminal spans. Those synthetic terminals are no longer opaque calls: each one now carries source handler, observed target handler where available, total VM-IP delta, status, site, and top bytes, then emits explicit `op_entry_NNN(vm)`, `next_entry = ...`, and `vm_ip += ...` statements. `make pseudocode-full` emits `vm_program_pseudocode_full.c`, currently 149520 lines: all 498 recovered VM basic blocks, 71642 `op_entry_NNN(vm)` calls, 205 referenced handler prototypes, and all 287 covered synthetic fallthroughs lifted into explicit recovered transitions. The only remaining `vm_unresolved_synthetic_tail` use in both program sketches is the default switch fallback for VM IPs outside the selected/recovered block set. `make source-bundle` writes `vm_recovered_source_bundle.c`, currently 156387 lines / 13 MiB, combining all 360 `op_entry_NNN` handler bodies with the full 498-block bytecode program and the 287 recovered synthetic transitions in one source-style artifact. This is the most readable C-shaped program artifact so far: native handler arithmetic, bytecode block order, bytes, VM IPs, expected next entries, decoded control edges, and recovered synthetic edges are now available together in the source bundle, with the separate `vm_handlers_pseudocode.c` and `vm_program_pseudocode_*.c` files kept as smaller views.

Coverage caveat: the C-like bytecode/CFG artifacts are dynamic-path reconstructions, not proof that every VM program path has been executed. `vm_trace_coverage_matrix.py` compares the raw available runs. The current primary `vmtail-wide-1m-w16` run observes 202 source handlers, 205 target handlers, and 71513 unique VM-IP starts; the shorter `wide`, `wide-w16`, `mode0-w16`, `mode2-w16`, and `state-wide-w16` runs each observe only 191 source handlers, 195 target handlers, and 39725 VM-IP starts, adding no source handlers beyond the primary and missing 11 primary sources. The static handler inventory is broader: the dispatch table and microcode catalog cover 360 handler entries, of which 202 have source events in the primary run. So static handler semantics are not limited to the no-network path, but the recovered VM bytecode listing is still scenario/path coverage until we add traces from other configs or force unexplored VM branches.

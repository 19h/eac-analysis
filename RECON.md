...
- `vm_synthetic_gap_live_in_roles.py`: joins live-in synthetic gap target expressions with GPR/scratch VMTAIL rows, names live registers, records the needed final native tail site/register, and consumes `mem_<reg>` qword fields when a memory-enabled tail trace reaches the gap. It now separates memory reads observed at any matching VM IP from reads captured at the exact final native tail site via `tail_event_site_match`.

...
`vm_synthetic_tail_lift.py` maps those suffix variants back to individual synthetic VM IPs. The current lift covers 313 VM IPs / 1570 suffix events: 843 events have the observed target entry encoded directly in the suffix, 1073 events have a recognized schema, 497 events remain observed-target-only, and 230 events match the longer `3d01`/`9d01` schema without a target word. The generated program/source sketches now include `synthetic tail lift:` comments on the synthetic edges they contain, so the C-like view shows schemas such as `8000+e801+target_u16@4`, `target_u16@0+e801`, and `e801+target_u16@2` next to the recovered transition. Fully encoded single-offset target tails are promoted to executable C-like target loads: direct block sketches use `next_entry = (int)U16(vm->ip + prefix + tail_offset)`, while handler-call sketches emit the post-prefix `U16(vm->ip + tail_offset)` after `op_entry_NNN(vm)`.

`vm_pseudocode_dump.py` is the first direct C-like reconstruction artifact. It renders the recovered VM IR as block functions over a `VMState { ip, state, flags, byte }`, translating `state0`, `flags0`, `u16_N`, `u32_N`, and byte operands into C-like expressions and emitting `next_entry = ...; vm->ip += ...;` for each recovered VM instruction. `make pseudocode` writes `dumps/vmtail-wide-1m-w16/vm_pseudocode_top.c`; the current file contains 60 hottest VM basic blocks / 12294 lines, with long blocks truncated to 32 recovered IR statements each. Its 21 selected synthetic terminal spans are now lifted into explicit source/target/delta comments plus `next_entry = ...` and `vm->ip += ...` statements; 19 of those selected synthetic spans also carry row-level synthetic-tail schemas, and 15 now emit direct `U16(vm->ip + offset)` target loads. The only remaining `vm_unresolved_synthetic_tail` use is the default switch fallback. This is not polished source yet, but it is now a concrete C-shaped view of recovered behavior instead of only TSVs.
...
`vm_program_pseudocode_dump.py` then renders the recovered bytecode program in terms of that handler layer. `make pseudocode` emits `vm_program_pseudocode_top.c`, currently 25843 lines: 80 hottest VM basic blocks, 6182 `op_entry_NNN(vm)` calls, 6153 exact-row `next_entry` fallback assignments, 154 referenced handler prototypes, 29 recovered synthetic terminal spans, 27 synthetic-tail lift comments, and 21 executable synthetic-tail target loads. Those synthetic terminals are no longer opaque calls: each one now carries source handler, observed target handler where available, total VM-IP delta, status, site, top bytes, and row-level tail schema where recovered, then emits explicit `op_entry_NNN(vm)`, `next_entry = ...`, and `vm_ip += ...` statements. `make pseudocode-full` emits `vm_program_pseudocode_full.c`, currently 292704 lines / 17 MiB: all 498 recovered VM basic blocks, 71642 `op_entry_NNN(vm)` calls, 71355 exact-row `next_entry` fallback assignments, 205 referenced handler prototypes, all 287 covered synthetic fallthroughs lifted into explicit recovered transitions, 261 synthetic-tail lift comments, and 212 executable synthetic-tail target loads. The only remaining `vm_unresolved_synthetic_tail` use in both program sketches is the default switch fallback for VM IPs outside the selected/recovered block set. `make source-bundle` writes `vm_recovered_source_bundle.c`, currently 299698 lines / 18 MiB, combining all 360 `op_entry_NNN` handler bodies with the full 498-block bytecode program, 287 recovered synthetic transitions, 261 row-level tail-schema comments, and 212 synthetic-tail target-load expressions in one source-style artifact. This is the most readable C-shaped program artifact so far: native handler arithmetic, bytecode block order, bytes, VM IPs, expected next entries, decoded control edges, recovered per-instruction `vm_ip` progress, recovered synthetic edges, synthetic suffix schemas, and direct suffix target loads are now available together in the source bundle, with the separate `vm_handlers_pseudocode.c` and `vm_program_pseudocode_*.c` files kept as smaller views.

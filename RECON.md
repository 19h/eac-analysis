...
The normal C check path is no longer tied to phony pseudocode aggregate prerequisites. `pseudocode-syntax-check`, `pseudocode-object-check`, and `pseudocode-link-check` now depend on concrete generated C file targets where possible, so unchanged checks skip bytecode catalog regeneration; the current fast route (`make -s pseudocode-syntax-check pseudocode-object-check pseudocode-link-check`) completed in 9.41 seconds after the ret-patch hidden bridge update.

...
- `dumps/vmtail-wide-1m-w16/vm_native_retdec_gap_queue.c`, `.tsv`, and `.md`: ranked native function backlog for semantic C recovery. It records 2755 queued `.text` functions, 738149 function-boundary uncovered bytes, and a tier mix of 2427 small, 247 medium, 77 large, and 4 huge/noisy targets; the C sidecar keeps all 2755 rows in the all-evidence file.
...
The all-evidence bundle also carries `vm_native_retdec_gap_queue.c` as a renamed sidecar. This is the current strategic backlog for native semantic C recovery: 2755 `.text` functions still have bytes not covered by semantic RetDec/native C sidecars, totaling 738149 function-boundary uncovered bytes. The queue is deliberately tiered for fast targeted work: 2427 small single-function targets, 247 medium targets, 77 large targets, and 4 huge/noisy targets that should be split or modeled before broad RetDec.

...
- `dumps/vmtail-wide-1m-w16/vm_pseudocode_top.c`: C-like decompiler sketch for the hottest recovered VM basic blocks, with `VMState`, `U8/U16/U32` accessors, state assignments, dispatch/IP updates, terminal edges, and explicit unresolved synthetic-tail calls.
...
`vm_program_pseudocode_dump.py` then renders the recovered bytecode program in terms of that handler layer. `make pseudocode` now also emits `vm_program_pseudocode_top.c`, currently 13488 lines: 80 hottest VM basic blocks, 6182 `op_entry_NNN(vm)` calls, 154 referenced handler prototypes, and 29 recovered synthetic terminal spans. Those synthetic terminals are no longer opaque calls: each one now carries source handler, observed target handler where available, total VM-IP delta, status, site, and top bytes, then emits explicit `op_entry_NNN(vm)`, `next_entry = ...`, and `vm_ip += ...` statements. The only remaining `vm_unresolved_synthetic_tail` use in this compact program is the default switch fallback for VM IPs outside the selected top blocks. This is the most readable C-shaped program artifact so far: native handler arithmetic lives in `vm_handlers_pseudocode.c`, while bytecode block order, bytes, VM IPs, expected next entries, decoded control edges, and recovered synthetic edges live in `vm_program_pseudocode_top.c`.

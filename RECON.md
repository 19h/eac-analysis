...
`vm_synthetic_gap_chain_slot_reconciliation.py` checks whether those focused first-hop targets are directly explained by the symbolic/table slot evidence. It joins all 57 replay steps to the transfer probe, table-memory probe, runtime table-memory probe, and sampled-control sidecars. This audit is intentionally conservative: matching would require the final table offset to equal `first_hop_target_entry * 8`. Current results show no direct slot match. The relation mix is 42 table offsets that are misaligned versus the focused target slot, 11 table offsets that are outside the 360-entry dispatch table, and 4 table offsets that point at a different dispatch slot. The class mix is 53 rows where table-read evidence rejects the focused first-hop as a direct slot and 4 rows where the table bytes are non-pointer data for that first-hop slot. This reinforces the current model: focused chains are observed control-flow provenance, while the residual table slot formulas are not yet statically reconciled into hard CFG.

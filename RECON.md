...
`vm_synthetic_gap_live_in_roles.py` narrows the live-in bucket by joining those 21 target expressions against the GPR/scratch VMTAIL trace and the focused memory-enabled start trace. The same-IP GPR/scratch trace still has start-register context for 11 of the 21 rows; 10 rows still lack that paired start event. Those 11 register rows are no longer anonymous: examples include source 175 at `0xd1445`, where `r13` is the current target handler entry 175 and `r9` is `frame+0xa`; source 195 at `0xd5ebc`, where `r10` is `frame+0x170` and `rdi` is constant `0x618`; and source 299 at `0x24bd2a`, where `r12` is constant `1` and `r14` is `frame+0xa`. The join also records the exact final native tail hooks needed for the dereferences: `175@0x9c3f4:r13`, `195@0xa0068:r10`, `278@0xb265e:r10`, `299@0xb708a:r12`, and `356@0xc2d35:rbx`.

`driver.c` has those five final-tail sites in the default VMTAIL hook set and adds gated `EAC_VMTAIL_MEM=1` qword reads from selected GPR pointers. The bounded `dumps/vmtail-mem-smoke/run.stderr` verifies `tail_mem=1` with `mem_r9`, `mem_rdi`, `mem_r14`, and related fields at the added sites. The focused `dumps/vmtail-mem-focus-starts/run.stderr` uses `EAC_VMTAIL_FOCUS_IPS` and `EAC_VMTAIL_STOP_AFTER_MATCHES=42` to reach all 21 live-in starts without a broad 12-minute scan. The regenerated role report now finds memory-enabled VMTAIL rows for all 21 live-in starts, two event-local dereferences that match the observed target handler, and one exact final-tail-site match in that start-focused probe: source 175 at start `0x34556c` reads `*r9=current_target(entry=175,off=0x9c08f)` at final site `0x9c3f4`. The other start-focused target-matching read is useful observed-site evidence but not an exact final-tail hit, and seven additional rows read a qword that does not match the observed target or lacks the paired start GPR event.

The exact native final-tail mechanics are now resolved separately by site-focused probes. `EAC_VMTAIL_FOCUS_SITES` captures the five final hooks directly and `vm_live_in_final_tail_site_probe.py` summarizes 234 exact-site VMTAIL events across sources 175, 195, 278, 299, and 356. Every row has the final jump register equal to the observed target and the expected dereference register equal to the same target: `175@0x9c3f4:r9=95/95`, `195@0xa0068:rdi=98/98`, `278@0xb265e:rbx=14/14`, `299@0xb708a:r14=7/7`, and `356@0xc2d35:r10=20/20`. This proves the unresolved live-in forms are ordinary exact-site qword loads from those registers, not unknown byte decoding. The remaining blocker is correlation/path forcing: connect those exact final-tail events back to the 21 synthetic gap start contexts, and rerun GPR/scratch coverage for the ten starts still absent from the paired register trace.

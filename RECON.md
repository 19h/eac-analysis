...
- `dumps/vmtail-wide-1m-w16/vm_native_retdec_gap_queue.c`, `.tsv`, and `.md`: ranked native function backlog for semantic C recovery. It records 2903 queued `.text` functions, 787695 function-boundary uncovered bytes, and a tier mix of 2575 small, 247 medium, 77 large, and 4 huge/noisy targets; the C sidecar keeps all 2903 rows in the all-evidence file.
...
The normal C check and manifest refresh paths are no longer tied to phony pseudocode/reconstruction aggregate prerequisites. `pseudocode-syntax-check`, `pseudocode-object-check`, `pseudocode-link-check`, and `c-reconstruction-manifest` now depend on concrete generated C/artifact file targets where possible, so unchanged checks skip bytecode catalog, synthetic probe, coverage, and RetDec sidecar regeneration. The current manifest gate (`make -s c-reconstruction-manifest`) completed in 35.08 seconds after adding the native function-inventory, native RetDec gap queue, and native gap RetDec batch00/01/02/03/04/05/06/07/08 sidecar checks.

...
`vm_synthetic_span_catalog.py` turns those byte-covered synthetic rows into an explicit semantic-gap catalog. The final file-backed trace has 1597 synthetic rows grouped into 148 source/target/status classes, with 996 terminal events linked back to covered synthetic block terminals. All groups are now typed: 134 groups / 1227 events are `known_prefix_plus_tail`, 6 groups / 23 events are `operand_footprint_only`, 5 groups / 343 events are `target_only_span`, and 3 groups / 4 events are `single_known_footprint`. By known-prefix source, 93 groups / 728 events have decoded long-branch operand prefixes, 47 groups / 506 events have exact handler-shape prefixes, and 3 groups / 20 events have sampled-operand prefixes. The biggest remaining hard semantic island is target-only entry 50, whose hidden spans cover 320 terminal events with no handler shape yet; most other high-volume synthetic spans are not raw unknowns but a known 10/11/13-byte prefix plus a 4/5/6/8/11-byte unresolved tail.

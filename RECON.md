...
- `dumps/vmtail-wide-1m-w16/vm_native_retdec_gap_queue.c`, `.tsv`, and `.md`: ranked native function backlog for semantic C recovery. It records 3023 queued `.text` functions, 841065 function-boundary uncovered bytes, and a tier mix of 2695 small, 247 medium, 77 large, and 4 huge/noisy targets; the C sidecar keeps all 3023 rows in the all-evidence file.
...
The all-evidence bundle also carries `vm_native_retdec_gap_queue.c` as a renamed sidecar. This is the current strategic backlog for native semantic C recovery: 3039 `.text` functions still have bytes not covered by semantic RetDec/native C sidecars, totaling 848913 function-boundary uncovered bytes. The queue is deliberately tiered for fast targeted work: 2711 small single-function targets, 247 medium targets, 77 large targets, and 4 huge/noisy targets that should be split or modeled before broad RetDec.

The all-evidence bundle also carries `vm_native_gap_retdec_batch00.c` as a renamed sidecar. This is the first fixed batch from that native gap queue: 16 selected top-ranked ranges decompiled in 2.02 seconds into 24 RetDec C functions with 24 address-range comments. The coverage audit now counts those ranges, so the native gap queue dropped from 3055 to 3039 rows and recovered executable bytes increased to 346721.
...
The normal C check and manifest refresh paths are no longer tied to phony pseudocode/reconstruction aggregate prerequisites. `pseudocode-syntax-check`, `pseudocode-object-check`, `pseudocode-link-check`, and `c-reconstruction-manifest` now depend on concrete generated C/artifact file targets where possible, so unchanged checks skip bytecode catalog, synthetic probe, coverage, and RetDec sidecar regeneration. The current manifest gate (`make -s c-reconstruction-manifest`) completed in 28.60 seconds after adding the native function-inventory, native RetDec gap queue, and native gap RetDec batch00 sidecar/checks.

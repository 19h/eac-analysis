...
- `vm_synthetic_gap_live_in_roles.py`: joins live-in synthetic gap target expressions with GPR/scratch VMTAIL rows, names live registers, records the needed final native tail site/register, and consumes `mem_<reg>` qword fields when a memory-enabled tail trace reaches the gap.
...
- `dumps/vmtail-wide-1m-w16/vm_synthetic_gap_live_in_roles.tsv`: live-register role join for the 21 live-in synthetic gap target expressions, now including final-tail hook site/register columns and dereference-read status columns.
- `dumps/vmtail-wide-1m-w16/vm_synthetic_gap_live_in_roles.md`: Markdown digest of found/missing GPR events, final-tail hook sites, and remaining event-local qword dereference blockers.
...
`vm_synthetic_gap_live_in_roles.py` narrows the live-in bucket by joining those 21 target expressions against the GPR/scratch VMTAIL trace. It finds same-IP GPR events for 11 of the 21 rows. All 11 still require an event-local qword dereference, but their live registers are no longer anonymous: examples include source 175 at `0xd1445`, where `r13` is the current target handler entry 175 and `r9` is `frame+0xa`; source 195 at `0xd5ebc`, where `r10` is `frame+0x170` and `rdi` is constant `0x618`; and source 299 at `0x24bd2a`, where `r12` is constant `1` and `r14` is `frame+0xa`. The join now also records the exact final native tail hooks needed for those dereferences: `175@0x9c3f4:r13`, `195@0xa0068:r10`, `278@0xb265e:r10`, `299@0xb708a:r12`, and `356@0xc2d35:rbx`. `driver.c` has those five sites in the default VMTAIL hook set and adds gated `EAC_VMTAIL_MEM=1` qword reads from selected GPR pointers; the bounded `dumps/vmtail-mem-smoke/run.stderr` verifies `tail_mem=1` with `mem_r9`, `mem_rdi`, `mem_r14`, and related fields at those sites. That smoke did not reach the 21 exact synthetic gap IPs, so the blocker is now narrower rather than solved: rerun a focused memory-enabled trace that reaches those starts, and rerun GPR/scratch coverage for the ten starts still absent from the available register trace.

...
- `vm_synthetic_gap_live_in_reentry_probe.py`: joins the remaining live-in synthetic starts with dynamic next-hooked-source stitch rows and exact final-tail source proof, classifying every row as comment-only unless a hidden-handler chain is statically replayed.
...
`vm_synthetic_gap_transfer_probe.py` now probes the 21 remaining synthetic-successor holes instead of leaving them as an undifferentiated unresolved bucket. One source-246 gap reduces to a symbolic long-control style transfer, `next = T[u16_0 << 0x3]` with `ip = u32_4`, and the zero-seeded concrete path lands on entry 354 at `+0x1870`; the former source-246 gap at `0x370249` is no longer in this bucket because the recovered hidden span now lands on a real split block at `0x370258`. The other 20 gaps are live-in central-dispatch register cases from sources 175, 195, 278, and 299, with target expressions such as `mem[r13, qword ptr [r9]]`, `mem[r10, qword ptr [rdi]]`, `mem[r12, qword ptr [r14]]`, and `mem[r10, qword ptr [rbx]]`. Source 356's former live-in row no longer appears in the remaining-gap set after embedded VMTAIL parsing recovers the `0x2c0468` block island. That means the next blocker is not byte decoding for those rows; it is reconstructing or tracing the live dispatch-slot/index/table registers at those exact starts. The full C-like program sketch and combined source bundle now carry transfer-probe comments at all 21 probe sites, but only 20 of those remain explicit unresolved-tail calls because `0x293689` is promoted through the hidden-chain reentry proof. The C reconstruction manifest records this split in its `gap_probe`, `gap_symbolic`, `gap_chain`, and `c_shape` sections.

`vm_synthetic_gap_live_in_roles.py` narrows the live-in bucket by joining those 20 target expressions against the GPR/scratch VMTAIL trace, the focused memory-enabled start trace, and the start/final-site intersection trace. The prioritized join now has start-register context for all 20 rows: scratch evidence is used when available, and `dumps/vmtail-mem-focus-starts/run.stderr` fills the starts absent from the scratch run. The rows are no longer anonymous: examples include source 175 at `0xd1445`, where `r13` is the current target handler entry 175 and `r9` is `frame+0xa`; source 195 at `0xd5ebc`, where `r10` is `frame+0x170` and `rdi` is constant `0x618`; source 278 at `0x12906f`, where `r10` is the dispatch table base and `rbx` is `frame+0x90`; and source 299 at `0x24bd2a`, where `r12` is constant `1` and `r14` is `frame+0xa`. The join also records the exact final native tail hooks needed for the dereferences: `175@0x9c3f4:r13`, `195@0xa0068:r10`, `278@0xb265e:r10`, and `299@0xb708a:r12`. Those 20 role rows, their matching final-tail site proof rows, dynamic live-in reentry rows, and all-static immediate-next reentry rows are now emitted beside the unresolved calls in `vm_program_pseudocode_full.c` and `vm_recovered_source_bundle.c`, with an explicit note that exact final-tail mechanics are mechanism evidence rather than path-complete coverage.

`driver.c` has those final-tail sites in the default VMTAIL hook set and adds gated `EAC_VMTAIL_MEM=1` qword reads from selected GPR pointers. The bounded `dumps/vmtail-mem-smoke/run.stderr` verifies `tail_mem=1` with `mem_r9`, `mem_rdi`, `mem_r14`, and related fields at the added sites. The focused `dumps/vmtail-mem-focus-starts/run.stderr` uses `EAC_VMTAIL_FOCUS_IPS` and `EAC_VMTAIL_STOP_AFTER_MATCHES=42` to reach the live-in starts without a broad 12-minute scan. The stricter `dumps/vmtail-mem-focus-start-final-sites/run.stderr` intersects the remaining starts with the four final source-class sites. The regenerated role report has memory-enabled VMTAIL rows for all remaining live-in starts, no missing start GPR rows, one event-local observed-site dereference match, and one exact final-tail-site match: source 175 at start `0x34556c` reads `*r9=current_target(entry=175,off=0x9c08f)` at final site `0x9c3f4`.

The exact native final-tail mechanics are resolved separately by site-focused probes. `EAC_VMTAIL_FOCUS_SITES` captures the four current final source-class hooks directly and `vm_live_in_final_tail_site_probe.py` summarizes 214 exact-site VMTAIL events across sources 175, 195, 278, and 299. Every row has the final jump register equal to the observed target and the expected dereference register equal to the same target: `175@0x9c3f4:r9=95/95`, `195@0xa0068:rdi=98/98`, `278@0xb265e:rbx=14/14`, and `299@0xb708a:r14=7/7`. This proves the unresolved live-in forms are ordinary exact-site qword loads from those registers, not unknown byte decoding.

`vm_synthetic_gap_live_in_reentry_probe.py` now joins the live-in role rows, source-level final-tail proof, and `vm_synthetic_gap_dynamic_stitch.py` reentry rows. It emits 21 event rows for 20 live-in starts because `0x24bd2a` has both an ambiguous event row and a byte-matched dynamic reentry row. All 20 live-in starts have byte-matched next-hooked-source evidence, and all 21 event rows have source-class exact final-tail proof. The class split is 18 `source_tail_mechanism_dynamic_reentry_not_promoted`, 1 `observed_site_mem_match_dynamic_reentry_not_promoted`, 1 `exact_tail_dynamic_reentry_not_promoted` at `0x34556c -> entry_169@0x11db12 -> 0x11db16`, and 1 `ambiguous_dynamic_reentry` for the alternate `0x24bd2a -> 0x249468` event. The hard-promotion count is intentionally zero: this probe tells the C what the next hooked source is, but it does not claim a hard CFG edge unless the hidden handlers between the live-in tail and the next hook are statically replayed as in the source-246 chain proof. The remaining blocker is path forcing or hidden-chain replay for those 20 live-in starts so the comment-only reentry evidence can become executable reconstructed control flow.

`vm_synthetic_gap_allstatic_reentry_probe.py` checks the same 21 live-in reentry rows against the broader `dumps/vmtail-allstatic/run.stderr` VMTAIL sequence. It parses 250000 all-static tail events, maps target offsets back to dispatch entries, finds live-in starts by `(vm_ip_off, source_entry)`, and compares the immediate next same-frame hooked tail with the expected dynamic reentry site, end VM IP, and target entry. The all-static window sees 12 rows / 11 unique starts and every seen row has an exact immediate-next match: 11 byte-matched dynamic reentry rows plus the ambiguous `0x24bd2a -> 0x249468` event. Nine rows are not observed before the all-static trace limit. These rows are now emitted as `all-static reentry` comments beside the unresolved calls in `vm_program_pseudocode_full.c` and `vm_recovered_source_bundle.c`; the hard-promotion count remains zero because this proves only the next configured all-static hook in that run, not a statically replayed hidden live-in chain.

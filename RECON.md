...
- `dumps/vmtail-wide-1m-w16/vm_binary_data_sections.c`, `.tsv`, and `.md`: native C-generated data carrier for program strings/data. It emits exact byte arrays for 22 allocatable non-executable ELF runtime data sections (`0xe63f7` bytes), metadata for 2 BSS/NOBITS sections, 2917 full string-reference rows / `0x10b64` printable bytes from emitted string-bearing sections, backing pointers into the exact byte arrays, C11 count assertions, and the raw 360-entry VM dispatch table from file offset `0xc3718`.

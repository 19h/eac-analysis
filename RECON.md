...
- `driver.c`: dlopen/dlsym harness for exported `x`, now also dumps the EAC memory maps, the output buffer, and the global sidecar context. Optional `EAC_DISPATCH_TRACE=1` patches selected dispatcher jumps to `int3`; `EAC_VMTAIL_TRACE=1` patches direct VM tail sites, including the five live-in synthetic-gap final tails. Both trace modes log frame state plus sixteen 16-bit `ip_w*` lookahead words when detail data is available; `EAC_VMTAIL_REGS=1` logs all GPRs at VM tail sites, `EAC_VMTAIL_SCRATCH=1` logs default and configured frame scratch qwords, and `EAC_VMTAIL_MEM=1` logs qwords at selected live GPR pointer values from `EAC_VMTAIL_MEM_REGS`. `EAC_VMTAIL_FOCUS_IPS`, `EAC_VMTAIL_FOCUS_SITES`, and `EAC_VMTAIL_STOP_AFTER_MATCHES` bound focused probes by VM IP and/or native tail site so live-in memory runs can stop after the targeted gap events instead of broad multi-minute traces.
...
- `vm_synthetic_gap_live_in_roles.py`: joins live-in synthetic gap target expressions with prioritized GPR/scratch and focused VMTAIL rows, names live registers, records the needed final native tail site/register, and consumes `mem_<reg>` qword fields when a memory-enabled tail trace reaches the gap. It now separates memory reads observed at any matching VM IP from reads captured at the exact final native tail site via `tail_event_site_match`, and records the run directory that supplied each start/tail event.
- `vm_live_in_final_tail_site_probe.py`: summarizes focused memory-enabled exact native-tail-site probes for the five live-in source classes, verifying that the final jump register and the expected `mem_<reg>` dereference both equal the observed native target at those sites.
- `vm_synthetic_gap_live_in_reentry_probe.py`: joins the 21 live-in synthetic starts with dynamic next-hooked-source stitch rows and exact final-tail source proof, classifying every row as comment-only unless a hidden-handler chain is statically replayed.
- `vm_synthetic_gap_allstatic_reentry_probe.py`: correlates those live-in reentry rows against the `dumps/vmtail-allstatic/run.stderr` VMTAIL sequence, checking whether the immediate next all-static hooked tail matches the dynamic reentry expectation.
...
- `dumps/vmtail-wide-1m-w16/vm_synthetic_gap_chain_probe.tsv`: hidden-handler-chain probe for ambiguous dynamic stitches, including the exact `0x293689 -> entry_308@0x293696 -> 0x2bec2e` reentry match.
...
- `dumps/vmtail-wide-1m-w16/vm_synthetic_gap_live_in_reentry_probe.tsv`: row-level join of live-in register-role evidence, source-level exact final-tail proof, and byte-matched dynamic next-hooked reentry.
- `dumps/vmtail-wide-1m-w16/vm_synthetic_gap_live_in_reentry_probe.md`: Markdown digest of live-in reentry classes, including the one row-exact final-tail reentry and the remaining comment-only dynamic reentries.
...
`vm_synthetic_tail_lift.py` maps those suffix variants back to individual synthetic VM IPs. The current lift covers 313 VM IPs / 1570 suffix events: 843 events have the observed target entry encoded directly in the suffix, 1 more event encodes the target across the known-prefix/tail boundary, 1073 events have a recognized tail schema, 603 events overlap a known decoded long-control branch at the same VM IP, another 123 events decode directly from an inline long-control prefix (`target_u32@+0,delta_u32@+4`) even when no catalog row covers that hidden/file-fill site, and 0 events remain observed-target-only. The generated program/source sketches now include `synthetic tail lift:` comments on the synthetic edges they contain, so the C-like view shows schemas such as `8000+e801+target_u16@4`, `target_u16@0+e801`, and `e801+target_u16@2` next to the recovered transition. Fully encoded single-offset target tails are promoted to executable C-like target loads: direct block sketches use `next_entry = (int)U16(vm->ip + prefix + tail_offset)`, while handler-call sketches emit the post-prefix `U16(vm->ip + tail_offset)` after `op_entry_NNN(vm)`. The lone boundary case is entry 5 at VM IP `0x11f1b9`: bytes `...6201e801` encode target `354` as `0x0162` split across the 9-byte handler prefix and 3-byte marker tail, so the full program emits `next_entry = (int)U16(vm->ip - 0x1)` after `op_entry_005(vm)`. The remaining long-tail family is no longer only a dangling schema: block comments now point at either the overlapping long-control branch, for example `316->171/+0x30` or `334->354/+0x2a5`, or the decoded inline long-control prefix, without pretending the shorter hidden edge is the whole branch.

`vm_pseudocode_dump.py` is the first direct C-like reconstruction artifact. It renders the recovered VM IR as block functions over a `VMState { ip, state, flags, byte }`, translating `state0`, `flags0`, `u16_N`, `u32_N`, and byte operands into C-like expressions and emitting `next_entry = ...; vm->ip += ...;` for each recovered VM instruction. `make pseudocode` writes `dumps/vmtail-wide-1m-w16/vm_pseudocode_top.c`; the current file contains 60 hottest VM basic blocks / 12594 lines, with long blocks truncated to 32 recovered IR statements each. Its 21 selected synthetic terminal spans are now lifted into explicit source/target/delta comments plus `next_entry = ...` and `vm->ip += ...` statements; 19 selected synthetic terminal spans carry row-level synthetic-tail schemas, 15 emit direct `U16(vm->ip + offset)` target loads, 17 internal synthetic rows now carry overlapping long-control comments, and 4 terminal synthetic edges carry decoded inline long-control-prefix comments. Decoded long-control rows in this view now use operand-driven C-like loads, `next_entry = (int)U32(vm->ip + 0x0)` and `signed_vm_delta_u32(U32(vm->ip + 0x4))`, instead of only hardcoded sidecar comments; the compact top view contains 3 of those decoded long-control load sites. The renderer emits clipped state expressions as comments rather than invalid C assignments, emits unsigned hex constants for wraparound-style arithmetic, emits forward declarations for selected block functions, and emits 92 concrete block calls, including 16 synthetic-successor calls recovered by applying lifted synthetic-span deltas. Self-loops and targets outside the selected compact block set stay as comments. The only remaining `vm_unresolved_synthetic_tail` use is the default switch fallback. This is not polished source yet, but it is now a concrete syntax-checkable C-shaped view of recovered behavior instead of only TSVs.

`vm_handler_pseudocode_dump.py` factors the VM ISA/operator layer out of the bytecode path. `make handler-pseudocode` emits `vm_handlers_pseudocode.c` for all 360 dispatch entries and `vm_handlers_hot_pseudocode.c` for the 80 hottest observed entries. The full file is 7247 lines and covers 166 `static_validated`, 155 `unobserved_static`, 13 `affine_dispatch_fallback`, 11 `partial`, 9 `sampled_only`, 3 `target_only`, and 3 `sampled_operand_lifted` handlers. Each `op_entry_NNN(VMState *vm)` carries native handler address, operand layout, native IP reads, state/flag expressions where safe, slot/IP updates where single-valued, observed top targets, validation status, and explicit comments for handlers not directly executed as source rows. When a single slot expression is recovered, the dump now normalizes it into `r.next_entry` as either a direct entry index or an 8-byte table offset; 130 handlers currently get such executable next-entry assignments. All 10 decoded long-control handlers now use the generic operand form in the handler body itself: `r.slot = U32(vm->ip + 0x0)`, `r.next_entry = vm_entry_from_slot_index(r.slot)`, and `vm->ip += signed_vm_delta_u32(U32(vm->ip + 0x4))`, instead of a representative observed delta. The 3 `sampled_operand_lifted` handlers keep their recovered source state/flag effects but deliberately do not emit a representative observed target/delta as generic handler behavior; their concrete sampled target/delta rows stay in the bytecode program layer. When a handler has no source-row execution but the native tail site contains a constant VM-IP update, the dump emits that static `vm->ip += ...` fallback; the current full file contains 157 such native-tail IP advances, including target-only entries 5, 50, and 80. Recovered handler expressions now emit unsigned hex constants, so `vm_handlers_pseudocode.c` passes a strict C syntax check with `-Wall -Wextra` and no diagnostic output. This is the static/global C-like layer; the block pseudocode remains the dynamic bytecode path layer.

`vm_program_pseudocode_dump.py` then renders the recovered bytecode program in terms of that handler layer. `make pseudocode` emits `vm_program_pseudocode_top.c`, currently 26370 lines: 80 hottest VM basic blocks, 80 forward block declarations, 6182 `op_entry_NNN(vm)` calls, 6153 exact-row `next_entry` fallback assignments, 154 referenced handler prototypes, 29 recovered synthetic terminal spans, 27 terminal synthetic-tail lift comments, 17 internal synthetic-tail lift comments, 6 decoded long-control prefix comments, 7 operand-driven decoded long-control target/delta loads, 21 executable synthetic-tail target loads, and 38 concrete block-to-block calls, including 17 synthetic-successor calls recovered by applying lifted synthetic-span deltas. Those synthetic terminals are no longer opaque calls: each one now carries source handler, observed target handler where available, total VM-IP delta, status, site, top bytes, row-level tail schema where recovered, and decoded inline long-control prefixes where present, then emits explicit `op_entry_NNN(vm)`, `next_entry = ...`, and `vm_ip += ...` statements. `make pseudocode-full` emits `vm_program_pseudocode_full.c`, currently 296595 lines / 17 MiB: all 499 recovered VM basic blocks, 499 forward block declarations, 71663 `op_entry_NNN(vm)` calls, 71355 exact-row `next_entry` fallback assignments, 205 referenced handler prototypes, all 287 covered synthetic fallthroughs lifted into explicit recovered transitions, 261 terminal synthetic-tail lift comments, 52 internal synthetic-tail lift comments, 48 decoded long-control prefix comments, all 146 decoded long-branch rows rendered with `U32(vm->ip + 0x0)` target loads plus signed `U32(vm->ip + 0x4)` VM-IP deltas, all 12 sampled-operand sidecar rows rendered with joined source-handler state effects, 497 concrete block-to-block calls, 266 synthetic-successor calls, 213 executable synthetic-tail target loads, 22 transfer-probe evidence comments, 22 dynamic-stitch evidence-site comments / 23 event-instance comments, 1 symbolic-successor audit comment / 1 candidate comment, 2 hidden-chain probe sites / 2 candidate comments / 1 resolved hidden-chain handler call, 21 live-in role evidence comments, 21 live-in row comments, 21 exact final-tail proof comments, 21 live-in reentry probe sites / 22 reentry comments, 21 all-static reentry probe sites / 22 all-static reentry comments plus 20 disabled observed-reentry bridge snippets beside the remaining unresolved successor sites. Of the sampled sidecar state effects, 7 are short enough to emit as `vm->state = ...` assignments and 5 are preserved as clipped comments instead of invalid C. The remaining selected/recovered-block fallback is the switch default plus 21 explicit `vm_unresolved_synthetic_tail(vm, 0x...)` calls at synthetic successors that do not land on recovered block starts or a hidden-chain reentry; those calls now carry transfer-probe classification, symbolic-slot successor audit, next-hooked-source sequence evidence, hidden-chain evidence, live-in/final-tail evidence, joined live-in reentry evidence, and all-static immediate-next evidence where available, and the lone recovered self-loop is retained as a CFG comment to avoid emitting a trivially infinite recursive C function. `make source-bundle` writes `vm_recovered_source_bundle.c`, currently 303621 lines / 18 MiB, combining all 360 `op_entry_NNN` handler bodies with the full 499-block bytecode program, 287 recovered synthetic transitions, 313 synthetic-tail lift comments, 52 overlap comments, 48 decoded-prefix comments, 156 decoded long-control operand loads across handler and program layers, 12 sampled-operand source-state sidecars, 3 sampled-operand-only handler caveats, 497 block-to-block calls, 21 explicit unresolved synthetic-successor calls, 213 synthetic-tail target-load expressions, 22 transfer-probe evidence sites, 22 dynamic-stitch evidence sites / 23 event comments, 1 symbolic-successor audit site / 1 candidate comment, 2 hidden-chain probe sites / 1 exact hidden-chain reentry, 21 live-in role evidence sites, 21 live-in role comments, 21 exact final-tail proof comments, 21 live-in reentry probe sites / 22 reentry comments, 21 all-static reentry probe sites / 22 all-static reentry comments plus 20 disabled observed-reentry bridge snippets in one source-style artifact. `make pseudocode-syntax-check` now regenerates all compact/full C-like pseudocode views and verifies that `vm_handlers_pseudocode.c`, `vm_path_handlers_pseudocode.c`, `vm_pseudocode_top.c`, `vm_program_pseudocode_top.c`, `vm_program_pseudocode_full.c`, and `vm_recovered_source_bundle.c` all pass warning-enabled C11 `-fsyntax-only` parsing with no diagnostic output; the current refresh passes under the configured compiler (`clang-21` on this host). `make pseudocode-object-check` additionally codegen-compiles the combined source bundle with `-O0 -fno-strict-aliasing` to `/tmp/eacsym-vm_recovered_source_bundle.o`; the current object is 5.5 MiB. `make pseudocode-link-check` links the same 18 MiB bundle with `vm_recovered_source_harness.c` into `/tmp/eacsym-vm_recovered_source_smoke` and runs a default-path smoke call through `vm_program_sketch`; the current linked executable is 5.5 MiB and exits successfully. `make c-reconstruction-manifest` runs the syntax, object, link-smoke, coverage-audit, and synthetic-successor-gap gates before writing `vm_c_reconstruction_manifest.tsv/md`; the current manifest records 360 handler functions, 560 path-specialized functions, 499 full program block functions, 71663 rendered handler calls, 497 block-to-block calls, 266 synthetic-successor calls, 21 explicit unresolved synthetic successor calls, 22 transfer-probe evidence sites, 22 dynamic-stitch evidence sites / 23 event comments, 1 symbolic-successor audit site, 2 hidden-chain probe sites / 1 exact hidden-chain reentry, 21 live-in/final-tail evidence sites, 21 live-in reentry probe sites / 22 reentry comments, 21 all-static reentry probe sites / 22 all-static reentry comments plus 20 disabled observed-reentry bridge snippets in both the full program sketch and source bundle. This is the most readable and mechanically C-shaped program artifact so far: native handler arithmetic, bytecode block order, bytes, VM IPs, expected next entries, decoded control edges, recovered per-instruction `vm_ip` progress, recovered synthetic edges, synthetic suffix schemas, direct/boundary suffix target loads, operand-driven long-control loads, sampled-operand source effects, long-control overlap/prefix annotations, transfer-probe gap classifications, symbolic-slot successor audits, hidden-handler-chain reentry proof, live-in role evidence, exact final-tail site proofs, next-hooked-source stitch evidence, and all-static immediate-next corroboration, plus disabled observed reentry bridges, are now available together in a source bundle that parses, codegens, links, smoke-runs, and has a reproducible manifest and remaining-gap report, with the separate `vm_handlers_pseudocode.c` and `vm_program_pseudocode_*.c` files kept as smaller views.

The program-layer C now also carries observed reentry bridges for the live-in synthetic gaps. These are guarded by `VM_ENABLE_OBSERVED_REENTRY_BRIDGES` and default to off: the default artifact still leaves the 21 unresolved synthetic-tail calls in place, but 20 of those sites now have a nearby disabled snippet that assigns the observed next hooked VM IP, calls the inferred next-source `op_entry_NNN`, falls through to the recovered destination block, and returns. The skipped row is `0x122e3c -> 0x122e42`, whose observed next end `0x2c0468` is not a recovered block start. The bridge comments name the dynamic event span, next-source entry/start, next end, observed target entry, destination block, and all-static corroboration status, so the C view exposes the observed execution path without promoting it to hard CFG until the hidden live-in handler chain is replayed statically.

`vm_synthetic_gap_transfer_probe.py` now probes the 22 remaining synthetic-successor holes instead of leaving them as an undifferentiated unresolved bucket. One source-246 gap reduces to a symbolic long-control style transfer, `next = T[u16_0 << 0x3]` with `ip = u32_4`, and the zero-seeded concrete path lands on entry 354 at `+0x1870`; the former source-246 gap at `0x370249` is no longer in this bucket because the recovered hidden span now lands on a real split block at `0x370258`. The other 21 gaps are live-in central-dispatch register cases from sources 175, 195, 278, 299, and 356, with target expressions such as `mem[r13, qword ptr [r9]]`, `mem[r10, qword ptr [rdi]]`, `mem[r12, qword ptr [r14]]`, `mem[r10, qword ptr [rbx]]`, and `mem[rbx, qword ptr [r10]]`. That means the next blocker is not byte decoding for those rows; it is reconstructing or tracing the live dispatch-slot/index/table registers at those exact starts. The full C-like program sketch and combined source bundle now carry transfer-probe comments at all 22 probe sites, but only 21 of those remain explicit unresolved-tail calls because `0x293689` is promoted through the hidden-chain reentry proof. The C reconstruction manifest records this split in its `gap_probe`, `gap_symbolic`, `gap_chain`, and `c_shape` sections.

`vm_synthetic_gap_live_in_roles.py` narrows the live-in bucket by joining those 21 target expressions against the GPR/scratch VMTAIL trace, the focused memory-enabled start trace, and the start/final-site intersection trace. The prioritized join now has start-register context for all 21 rows: scratch evidence is used when available, and `dumps/vmtail-mem-focus-starts/run.stderr` fills the starts absent from the scratch run. The rows are no longer anonymous: examples include source 175 at `0xd1445`, where `r13` is the current target handler entry 175 and `r9` is `frame+0xa`; source 195 at `0xd5ebc`, where `r10` is `frame+0x170` and `rdi` is constant `0x618`; source 278 at `0x12906f`, where `r10` is the dispatch table base and `rbx` is `frame+0x90`; and source 299 at `0x24bd2a`, where `r12` is constant `1` and `r14` is `frame+0xa`. The join also records the exact final native tail hooks needed for the dereferences: `175@0x9c3f4:r13`, `195@0xa0068:r10`, `278@0xb265e:r10`, `299@0xb708a:r12`, and `356@0xc2d35:rbx`. Those 21 role rows, their matching final-tail site proof rows, dynamic live-in reentry rows, and all-static immediate-next reentry rows are now emitted beside the unresolved calls in `vm_program_pseudocode_full.c` and `vm_recovered_source_bundle.c`, with an explicit note that exact final-tail mechanics are mechanism evidence rather than path-complete coverage.

`driver.c` has those five final-tail sites in the default VMTAIL hook set and adds gated `EAC_VMTAIL_MEM=1` qword reads from selected GPR pointers. The bounded `dumps/vmtail-mem-smoke/run.stderr` verifies `tail_mem=1` with `mem_r9`, `mem_rdi`, `mem_r14`, and related fields at the added sites. The focused `dumps/vmtail-mem-focus-starts/run.stderr` uses `EAC_VMTAIL_FOCUS_IPS` and `EAC_VMTAIL_STOP_AFTER_MATCHES=42` to reach all 21 live-in starts without a broad 12-minute scan. The stricter `dumps/vmtail-mem-focus-start-final-sites/run.stderr` intersects the 21 starts with the five final sites. The regenerated role report now has memory-enabled VMTAIL rows for all 21 starts, no missing start GPR rows, two event-local dereferences that match the observed target handler, and one exact final-tail-site match: source 175 at start `0x34556c` reads `*r9=current_target(entry=175,off=0x9c08f)` at final site `0x9c3f4`. The start/final intersection also confirms exact-site observations at `0x33710f`, `0x33a6cc`, and `0x33d2d9`, but those are not the owning final-tail sites for those transfer-probe rows, so they stay classified as observed-site evidence rather than solved row successors. The remaining live-in role split is 12 unresolved dereference reads, 7 observed target mismatches, 1 observed-site target match, and 1 exact final-tail target match.

The exact native final-tail mechanics are now resolved separately by site-focused probes. `EAC_VMTAIL_FOCUS_SITES` captures the five final hooks directly and `vm_live_in_final_tail_site_probe.py` summarizes 234 exact-site VMTAIL events across sources 175, 195, 278, 299, and 356. Every row has the final jump register equal to the observed target and the expected dereference register equal to the same target: `175@0x9c3f4:r9=95/95`, `195@0xa0068:rdi=98/98`, `278@0xb265e:rbx=14/14`, `299@0xb708a:r14=7/7`, and `356@0xc2d35:r10=20/20`. This proves the unresolved live-in forms are ordinary exact-site qword loads from those registers, not unknown byte decoding.

`vm_synthetic_gap_live_in_reentry_probe.py` now joins the live-in role rows, source-level final-tail proof, and `vm_synthetic_gap_dynamic_stitch.py` reentry rows. It emits 22 event rows for 21 live-in starts because `0x24bd2a` has both an ambiguous event row and a byte-matched dynamic reentry row. All 21 live-in starts have at least one byte-matched next-hooked-source reentry, and all 22 event rows have source-class exact final-tail proof. The class split is 19 `source_tail_mechanism_dynamic_reentry_not_promoted`, 1 `observed_site_mem_match_dynamic_reentry_not_promoted`, 1 `exact_tail_dynamic_reentry_not_promoted` at `0x34556c -> entry_169@0x11db12 -> 0x11db16`, and 1 `ambiguous_dynamic_reentry` for the alternate `0x24bd2a -> 0x249468` event. The hard-promotion count is intentionally zero: this probe tells the C what the next hooked source is, but it does not claim a hard CFG edge unless the hidden handlers between the live-in tail and the next hook are statically replayed as in the source-246 chain proof. The remaining blocker is path forcing or hidden-chain replay for those 21 live-in starts so the comment-only reentry evidence can become executable reconstructed control flow.

`vm_synthetic_gap_allstatic_reentry_probe.py` checks the same 22 live-in reentry rows against the broader `dumps/vmtail-allstatic/run.stderr` VMTAIL sequence. It parses 250000 all-static tail events, maps target offsets back to dispatch entries, finds live-in starts by `(vm_ip_off, source_entry)`, and compares the immediate next same-frame hooked tail with the expected dynamic reentry site, end VM IP, and target entry. The all-static window sees 12 rows / 11 unique starts and every seen row has an exact immediate-next match: 11 byte-matched dynamic reentry rows plus the ambiguous `0x24bd2a -> 0x249468` event. Ten rows are not observed before the all-static trace limit. These rows are now emitted as `all-static reentry` comments beside the unresolved calls in `vm_program_pseudocode_full.c` and `vm_recovered_source_bundle.c`; the hard-promotion count remains zero because this proves only the next configured all-static hook in that run, not a statically replayed hidden live-in chain.
...
`vm_synthetic_gap_dynamic_stitch.py` adds a sequence-side view of the same holes. It scans the raw `run.stderr` VMTAIL stream from each synthetic start and infers where execution next reappears when the next hooked tail site has a fixed handler delta and the file bytes at `next_end - delta` match that handler's known exact byte form. The current stitch has 23 event-instance rows for the 22 gap starts because one source-299 gap was seen twice: 21 rows / 21 unique starts stitch to a next hooked source start, and 2 rows remain ambiguous (`0x24bd2a` and `0x293689`) at this stitch-only layer. The resolved rows show that many `target_footprint_of_6/10` gaps are not just six or ten missing bytes; they jump across larger dynamic spans before the next hooked handler, for example source 175 at `0xd1445` reappears at entry 310 start `0xd4893` (`+0x344e`), source 299 at `0x122e3c` reappears at entry 169 start `0x2c0464` (`+0x19d628`), and source 356 at `0x2c045a` reappears at entry 310 start `0x122c85` (`-0x19d7d5`). The full C-like program sketch and combined source bundle now carry these rows as comments next to each unresolved synthetic-tail call or hidden-chain promotion, with the same caveat: this stitch is dynamic sequence evidence, not proof that no unhooked handlers occurred in between.

`vm_synthetic_gap_chain_probe.py` follows up on the stitch-only ambiguities by replaying candidate next-source handlers at short offsets after the synthetic start. It emits 2 rows: source 299 at `0x24bd2a` has a target-only hint through entry 323 at `0x24bd57` (`pred_entry=169`, but `pred_end=0x24bd5b` versus dynamic `0x249468`), so it remains unresolved; source 246 at `0x293689` has an exact hidden-chain match through entry 308 at `0x293696`, whose static transfer predicts `entry=171`, `delta=+0x2b598`, and `pred_end=0x2bec2e`, exactly matching the next hooked VMTAIL event. The program C now emits `op_entry_246(vm)`, advances to `0x293696`, emits `op_entry_308(vm)`, advances by `0x2b598`, and calls `prog_bb_0205 @ 0x2bec2e` instead of leaving `0x293694` as an unresolved synthetic tail. This reduces explicit unresolved synthetic-tail calls in the full program and source bundle from 22 to 21 while preserving the conflicting zero-seed symbolic candidate as an audit comment.

Coverage caveat: the C-like bytecode/CFG artifacts are dynamic-path reconstructions, not proof that every VM program path has been executed. `vm_trace_coverage_matrix.py` now auto-discovers 32 available run directories: 12 have `vm_instruction_trace.tsv` bytecode-path rows, 8 of those are concrete instruction-trace scenarios before synthetic fill sidecars, and 20 are run-only register/scratch/memory-context evidence with no instruction-trace rows. Six run-only rows are memory-context traces: the bounded `EAC_VMTAIL_MEM=1` smoke, the focused live-in start probe, the start/final-site intersection probe, the all-final-sites exact-tail probe, and the two single-site rare-tail probes. The concrete instruction traces cover `x()` modes 0, 1, and 2, but none adds source handlers beyond the primary long `vmtail-wide-1m-w16` run; six concrete traces miss one or more primary sources. The primary run observes 202 source handlers, 205 target handlers, and 71513 unique VM-IP starts. Synthetic file/hidden/frontier/footprint fill sidecars raise the instruction-trace union to 205 source handlers and 71800 VM-IP starts, but those rows are derived coverage, not independent runtime configs. The static handler inventory is broader: the dispatch table, transition model, state slice, microcode catalog, and handler pseudocode cover all 360 handler entries, including 155 entries not seen as primary-trace targets. So static handler semantics, handler pseudocode, operand-layout recovery, and dispatch-table inventory are not limited to the no-network path. The recovered VM bytecode listing still is scenario/path coverage until we add traces from other configs, enable/shape network-dependent paths, or force unexplored VM branches through static/symbolic exploration.

...
`vm_program_pseudocode_dump.py` then renders the recovered bytecode program in terms of that handler layer. `make pseudocode` emits `vm_program_pseudocode_top.c`, currently 25918 lines: 80 hottest VM basic blocks, 6182 `op_entry_NNN(vm)` calls, 6153 exact-row `next_entry` fallback assignments, 154 referenced handler prototypes, 29 recovered synthetic terminal spans, 27 terminal synthetic-tail lift comments, 17 internal synthetic-tail lift comments, 6 decoded long-control prefix comments, 7 operand-driven decoded long-control target/delta loads, and 21 executable synthetic-tail target loads. Those synthetic terminals are no longer opaque calls: each one now carries source handler, observed target handler where available, total VM-IP delta, status, site, top bytes, row-level tail schema where recovered, and decoded inline long-control prefixes where present, then emits explicit `op_entry_NNN(vm)`, `next_entry = ...`, and `vm_ip += ...` statements. `make pseudocode-full` emits `vm_program_pseudocode_full.c`, currently 293205 lines / 17 MiB: all 498 recovered VM basic blocks, 71642 `op_entry_NNN(vm)` calls, 71355 exact-row `next_entry` fallback assignments, 205 referenced handler prototypes, all 287 covered synthetic fallthroughs lifted into explicit recovered transitions, 261 terminal synthetic-tail lift comments, 52 internal synthetic-tail lift comments, 48 decoded long-control prefix comments, all 146 decoded long-branch rows rendered with `U32(vm->ip + 0x0)` target loads plus signed `U32(vm->ip + 0x4)` VM-IP deltas, and 213 executable synthetic-tail target loads. The only remaining `vm_unresolved_synthetic_tail` use in both program sketches is the default switch fallback for VM IPs outside the selected/recovered block set. `make source-bundle` writes `vm_recovered_source_bundle.c`, currently 300198 lines / 18 MiB, combining all 360 `op_entry_NNN` handler bodies with the full 498-block bytecode program, 287 recovered synthetic transitions, 313 synthetic-tail lift comments, 52 overlap comments, 48 decoded-prefix comments, 146 decoded long-control operand loads, and 213 synthetic-tail target-load expressions in one source-style artifact. This is the most readable C-shaped program artifact so far: native handler arithmetic, bytecode block order, bytes, VM IPs, expected next entries, decoded control edges, recovered per-instruction `vm_ip` progress, recovered synthetic edges, synthetic suffix schemas, direct/boundary suffix target loads, operand-driven long-control loads, and long-control overlap/prefix annotations are now available together in the source bundle, with the separate `vm_handlers_pseudocode.c` and `vm_program_pseudocode_*.c` files kept as smaller views.

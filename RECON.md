...
- `vm_binary_data_sections_dump.c`: native C ELF scanner/generator that carries allocatable non-executable runtime sections, BSS metadata, string-reference previews, and the raw VM dispatch-table offsets into C source form.
...
- `dumps/vmtail-wide-1m-w16/vm_binary_data_sections.c`, `.tsv`, and `.md`: native C-generated data carrier for program strings/data. It emits exact byte arrays for 22 allocatable non-executable ELF runtime data sections (`0xe63f7` bytes), metadata for 2 BSS/NOBITS sections, 2905 string-reference preview rows from emitted string-bearing sections, and the raw 360-entry VM dispatch table from file offset `0xc3718`.
...
The normal C check and manifest refresh paths are no longer tied to phony pseudocode/reconstruction aggregate prerequisites. `pseudocode-syntax-check`, `pseudocode-object-check`, `pseudocode-link-check`, and `c-reconstruction-manifest` now depend on concrete generated C/artifact file targets where possible, so unchanged checks skip bytecode catalog, synthetic probe, coverage, and RetDec sidecar regeneration. The current integrated fast route (`make -s static-only-tier3-shared handler-pseudocode source-bundle all-evidence-bundle pseudocode-syntax-check pseudocode-object-check pseudocode-link-check c-reconstruction-manifest`) completed in 15.89 seconds after adding the tier3 static-only shared model layer.

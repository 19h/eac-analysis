...
`vm_synthetic_gap_symbolic_successors.py` audits the two source-246 symbolic-slot candidates before promoting them into CFG edges. Both are still zero-seeded static candidates, not dynamic proof. The `0x293689` candidate is `entry=354, delta=+0x1870`, giving concrete destination `0x294ef9`, which is outside the currently recovered bytecode rows and disagrees with the next hooked VMTAIL end `0x2bec2e`. The `0x370249` candidate is `entry=165, delta=+0x1a47`, giving concrete destination `0x371c90`; that exact row is recovered and its source entry matches 165, but it is inside existing block 496 rather than at a block entry, and it disagrees with the next hooked VMTAIL end `0x370258`. The full C-like program and bundle now carry this audit directly at those two unresolved calls, including the candidate destination, destination row/block status, dynamic next-end mismatch, and `mid_block_split_needed` / `uncovered_dest` status. So the right next step is either a mid-block split plus stronger dynamic proof for `0x371c90`, or a separate forced trace/symbolic path; the current C stays conservative and leaves these as annotated unresolved tails.

`vm_synthetic_gap_dynamic_stitch.py` adds a sequence-side view of the same holes. It scans the raw `run.stderr` VMTAIL stream from each synthetic start and infers where execution next reappears when the next hooked tail site has a fixed handler delta and the file bytes at `next_end - delta` match that handler's known exact byte form. The current stitch has 24 event-instance rows for the 23 gap starts because one source-299 gap was seen twice: 21 rows / 21 unique starts stitch to a next hooked source start, 3 rows still have an ambiguous stitch, and only two unique starts (`0x293689`, `0x370249`) have no dynamic stitch at all. The resolved rows show that many `target_footprint_of_6/10` gaps are not just six or ten missing bytes; they jump across larger dynamic spans before the next hooked handler, for example source 175 at `0xd1445` reappears at entry 310 start `0xd4893` (`+0x344e`), source 299 at `0x122e3c` reappears at entry 169 start `0x2c0464` (`+0x19d628`), and source 356 at `0x2c045a` reappears at entry 310 start `0x122c85` (`-0x19d7d5`). The full C-like program sketch and combined source bundle now carry these rows as comments next to each unresolved synthetic-tail call, with the same caveat: this stitch is dynamic sequence evidence, not proof that no unhooked handlers occurred in between.

...
The all-evidence bundle also carries `vm_native_retdec_gap_queue.c` as a renamed sidecar. This is the current strategic backlog for native semantic C recovery: 2626 `.text` functions still have bytes not covered by semantic RetDec/native C sidecars, totaling 703607 function-boundary uncovered bytes. The queue is deliberately tiered for fast targeted work: 2298 small single-function targets, 247 medium targets, 77 large targets, and 4 huge/noisy targets that should be split or modeled before broad RetDec.

...
- `vm_synthetic_gap_live_in_roles.py`: joins live-in synthetic gap target expressions with GPR/scratch VMTAIL rows, naming live registers as current-target, frame-relative, table-related, constant, or unknown values.
...
`vm_synthetic_gap_live_in_roles.py` narrows the live-in bucket by joining those 21 target expressions against the GPR/scratch VMTAIL trace. It finds same-IP GPR events for 12 of the 21 rows. All 12 still require an event-local qword dereference, but their live registers are no longer anonymous: examples include source 175 at `0xd1445`, where `r13` is the current target handler entry 175 and `r9` is `frame+0xa`; source 195 at `0xd5ebc`, where `r10` is `frame+0x170` and `rdi` is constant `0x618`; and source 299 at `0x24bd2a`, where `r12` is constant `1` and `r14` is `frame+0xa`. Nine rows still lack a GPR/scratch event at the synthetic start in the available register trace, covering sources 175, 195, 278, 299, and 356. So the next blocker is now precise: trace the qword memory operands at those VMTAIL events, plus rerun GPR/scratch coverage for the nine missing starts.

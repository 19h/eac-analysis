...
`vm_program_pseudocode_dump.py` then renders the recovered bytecode program in terms of that handler layer. `make pseudocode` now also emits `vm_program_pseudocode_top.c`, currently 13363 lines: 80 hottest VM basic blocks, 6153 `op_entry_NNN(vm)` calls, 144 referenced handler prototypes, and 30 explicit `vm_unresolved_synthetic_tail(vm, ...)` calls. This is the most readable C-shaped program artifact so far: native handler arithmetic lives in `vm_handlers_pseudocode.c`, while bytecode block order, bytes, VM IPs, expected next entries, and decoded control edges live in `vm_program_pseudocode_top.c`.

...
`vm_synthetic_gap_live_snapshot_transfer_probe.py` generalizes that check to all 15 default-unresolved starts. It replays each unresolved source handler from selected live VMTAIL rows, including the focused residual full-GPR runs and the `vmtail-fakenet-w16` alternate/config run where matching rows exist but no GPR fields were logged. The current result emits 31 live rows covering all 15 unresolved starts: 24 rows have `full_gpr_snapshot` evidence and 7 are explicitly weaker `frame_only_snapshot` rows. All 31 still end at `unknown_target` because the final dispatch-table read remains symbolic, misaligned, or out of range; none is promoted into a hard CFG edge. The source-299 unknown branch is removed in 6 full-GPR rows, while the one fakenet source-299 row stays branch-unknown because that run lacks `r8`. Twenty rows have the next raw event equal to the observed first hop, two hit the terminal reentry, seven differ from the family chain, and two stop at the run limit. This makes the coverage answer sharper: the unresolved starts are not merely artifacts of the primary no-network trace, but the alternate/config evidence is weaker unless it carries registers.

...
`vm_program_pseudocode_dump.py` then renders the recovered bytecode program in terms of that handler layer. `make pseudocode` emits `vm_program_pseudocode_top.c`, currently 26363 lines: 80 hottest VM basic blocks, 80 forward block declarations, 38 concrete block-to-block calls, and explicit handler-call/`next_entry`/`vm_ip` statements for selected recovered rows and synthetic terminals. `make pseudocode-full` emits `vm_program_pseudocode_full.c`, currently 296301 lines / 19 MiB: all 499 recovered VM basic blocks, 499 forward block declarations, 71652 recovered bytecode operations rendered as `op_entry_NNN(vm)` calls, 71364 exact-row `next_entry` fallback assignments, 267 synthetic-successor calls, 214 executable synthetic-tail target loads, 21 transfer-probe evidence sites, 21 dynamic-stitch evidence sites / 22 event comments, 1 symbolic-successor audit site, and 2 hidden-chain probe sites with 1 exact hidden-chain reentry promoted to executable handler/block calls. The current full sketch has 20 explicit `vm_unresolved_synthetic_tail(vm, 0x...)` calls for synthetic successors that still do not land on recovered block starts or a statically replayed hidden chain; those calls carry transfer-probe, dynamic-stitch, symbolic-successor, and hidden-chain comments where available, and the lone recovered self-loop is retained as a CFG comment to avoid emitting a trivially infinite recursive C function.

`make source-bundle` writes `vm_recovered_source_bundle.c`, currently 303327 lines / 20 MiB, combining all 360 `op_entry_NNN` handler bodies with the full 499-block bytecode program, 287 recovered synthetic transitions, 12 sampled-operand source-state sidecars, 3 sampled-operand-only handler caveats, 478 block-to-block calls, 20 explicit unresolved synthetic-successor calls, 214 synthetic-tail target-load expressions, 21 transfer-probe evidence sites, 21 dynamic-stitch evidence sites / 22 event comments, 1 symbolic-successor audit site / 1 candidate comment, and 2 hidden-chain probe sites / 1 exact hidden-chain reentry. `make c-reconstruction-manifest` now regenerates the raw instruction trace, unique-instruction table, instruction lift, sampled recovery, file atlas, file-fill sidecars, IR, basic blocks, C sketches, syntax/object/link gates, coverage audit, and manifest in dependency order, so stale trace rows no longer stop at the TSV boundary. The current manifest records 360 handler functions, 120 table-offset handler next-entry normalizations, 10 direct-slot handler normalizations, 560 path-specialized functions, 499 full program block functions, 71652 rendered handler calls, 478 block-to-block calls, 267 synthetic-successor calls, 20 explicit unresolved synthetic successor calls, 21 transfer-probe evidence sites, 21 dynamic-stitch evidence sites / 22 event comments, 1 symbolic-successor audit site, and 2 hidden-chain probe sites / 1 exact hidden-chain reentry. Live-in role, final-tail proof, live-in reentry, all-static reentry, and observed bridge counts are now zero because the refreshed static transfer replay no longer classifies those 20 gaps as live-in register tails. This is the most readable and mechanically C-shaped program artifact so far: native handler arithmetic, bytecode block order, bytes, VM IPs, expected next entries, decoded control edges, recovered per-instruction `vm_ip` progress, recovered synthetic edges, synthetic suffix schemas, direct/boundary suffix target loads, operand-driven long-control loads, sampled-operand source effects, long-control overlap/prefix annotations, transfer-probe gap classifications, symbolic-slot successor audits, and hidden-handler-chain reentry proof are available together in a source bundle that parses, codegens, links, smoke-runs, and has a reproducible manifest and remaining-gap report, with the separate `vm_handlers_pseudocode.c` and `vm_program_pseudocode_*.c` files kept as smaller views.

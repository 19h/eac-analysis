...
`vm_program_pseudocode_dump.py` then renders the recovered bytecode program in terms of that handler layer. `make pseudocode` emits `vm_program_pseudocode_top.c`, currently 26377 lines: 80 hottest VM basic blocks, 80 forward block declarations, 38 concrete block-to-block calls, and explicit handler-call/`next_entry`/`vm_ip` statements for selected recovered rows and synthetic terminals. `make pseudocode-full` emits `vm_program_pseudocode_full.c`, currently 296760 lines / 19 MiB: all 499 recovered VM basic blocks, 499 forward block declarations, 71672 rendered `op_entry_NNN(vm)` call occurrences, 71364 exact-row `next_entry` fallback assignments, 498 concrete block-to-block calls, 267 synthetic-successor calls, 214 executable synthetic-tail target loads, 21 transfer-probe evidence sites, 21 dynamic-stitch evidence sites / 22 event comments, 1 symbolic-successor audit site, 21 hidden-chain/next-hook probe sites / 22 comments, 20 residual-audit sites / 20 residual promotion-state comments, 20 concrete-state audit sites / 20 concrete replay comments, 20 live-context audit sites / 20 comments, 20 table-read diagnostic sites / 20 comments, 20 table-memory probe sites / 20 comments, and 20 sampled-control correlation sites / 20 comments. The chain-probe rows split into 20 dynamic next-hook handlers whose static transfer predicts the observed next target/end, 1 target-only hidden-chain hint, and 1 exact hidden-chain reentry promoted to executable handler/block calls. The current full sketch has 20 explicit `vm_unresolved_synthetic_tail(vm, 0x...)` calls for synthetic successors that still do not land on recovered block starts or a statically replayed hidden chain; each dynamically stitched unresolved call now also has a disabled-by-default `VM_ENABLE_OBSERVED_REENTRY_BRIDGES` snippet that follows the observed next hooked source and recovered destination block. Those snippets are sequence evidence, not hard CFG promotions, and the lone recovered self-loop is retained as a CFG comment to avoid emitting a trivially infinite recursive C function.

`make source-bundle` writes `vm_recovered_source_bundle.c`, currently 303786 lines / 20 MiB, combining all 360 `op_entry_NNN` handler bodies with the full 499-block bytecode program, 287 recovered synthetic transitions, 12 sampled-operand source-state sidecars, 3 sampled-operand-only handler caveats, 498 block-to-block calls, 20 explicit unresolved synthetic-successor calls, 214 synthetic-tail target-load expressions, 21 transfer-probe evidence sites, 21 dynamic-stitch evidence sites / 22 event comments, 1 symbolic-successor audit site / 1 candidate comment, 21 hidden-chain/next-hook probe sites / 22 comments, 20 residual-audit sites / 20 residual promotion-state comments, 20 concrete-state audit sites / 20 concrete replay comments, 20 live-context audit sites / 20 comments, 20 table-read diagnostic sites / 20 comments, 20 table-memory probe sites / 20 comments, 20 sampled-control correlation sites / 20 comments, 1 exact hidden-chain reentry, and 20 disabled observed reentry bridges generated from byte-matched dynamic stitch rows. `make c-reconstruction-manifest` now regenerates the raw instruction trace, unique-instruction table, instruction lift, sampled recovery, file atlas, file-fill sidecars, IR, basic blocks, C sketches, syntax/object/link gates, coverage audit, synthetic-gap residual audit, concrete-state residual audit, live-context residual audit, table-read diagnostic, table-memory probe, sampled-control correlation, and manifest in dependency order, so stale trace rows no longer stop at the TSV boundary. The current manifest records 360 handler functions, 120 table-offset handler next-entry normalizations, 10 direct-slot handler normalizations, 560 path-specialized functions, 499 full program block functions, 71672 rendered handler-call occurrences, 498 block-to-block calls, 267 synthetic-successor calls, 20 explicit unresolved synthetic successor calls, 21 transfer-probe evidence sites, 21 dynamic-stitch evidence sites / 22 event comments, 1 symbolic-successor audit site, 21 hidden-chain/next-hook probe sites / 22 comments, 1 exact hidden-chain reentry, 20 dynamic next-hook static-transfer validations, 20 residual-audit rows, 20 concrete-state audit rows, 20 live-context audit rows, 20 table-read diagnostic rows, 20 table-memory probe rows, 20 sampled-control correlation rows, 20 residual comments in both the full program sketch and bundle, 20 concrete-state comments in both the full program sketch and bundle, 20 live-context comments in both, 20 table-read diagnostic comments in both, 20 table-memory probe comments in both, 20 sampled-control correlation comments in both, and 20 disabled observed reentry bridge snippets in both the full program sketch and bundle. Live-in role, final-tail proof, live-in reentry, and all-static reentry counts are now zero because the refreshed static transfer replay no longer classifies those 20 gaps as live-in register tails. This is the most readable and mechanically C-shaped program artifact so far: native handler arithmetic, bytecode block order, bytes, VM IPs, expected next entries, decoded control edges, recovered per-instruction `vm_ip` progress, recovered synthetic edges, synthetic suffix schemas, direct/boundary suffix target loads, operand-driven long-control loads, sampled-operand source effects, long-control overlap/prefix annotations, transfer-probe gap classifications, symbolic-slot successor audits, hidden-handler-chain reentry proof, dynamic next-hook validation, residual promotion-state audit, concrete-state replay audit, live-context replay audit, final table-read diagnostics, file-backed table-offset probes, sampled-control correlation, and disabled observed next-hook bridges are available together in a source bundle that parses, codegens, links, smoke-runs, and has a reproducible manifest and remaining-gap report, with the separate `vm_handlers_pseudocode.c` and `vm_program_pseudocode_*.c` files kept as smaller views.

`vm_synthetic_gap_transfer_probe.py` now probes the 21 remaining synthetic-successor holes instead of leaving them as an undifferentiated unresolved bucket. After the Python static transfer replayers learned the same restore-trampoline rule as the native fast model, simple register `push`/`pop` operands are preserved instead of clobbering seeded or symbolic registers. That changes the former 20 live-in tail rows into normal symbolic dispatch expressions: all 21 probe rows now classify as `symbolic_slot_expr`. One source-246 gap reduces to `next = T[u16_0 << 0x3]` with `ip = u32_4`, and the zero-seeded concrete path lands on entry 354 at `+0x1870`; the other 20 remain symbolic slot/IP formulas whose final target is not concretized under zero seed. The former source-246 gap at `0x370249` is no longer in this bucket because the recovered hidden span lands on the split block at `0x370258`, and source 356's former row no longer appears after embedded VMTAIL parsing recovers the `0x2c0468` block island. The full C-like program sketch and combined source bundle carry transfer-probe comments at all 21 probe sites, but only 20 of those remain explicit unresolved-tail calls because `0x293689` is promoted through the hidden-chain reentry proof.

`vm_synthetic_gap_live_in_roles.py` now emits only its header row: there are 0 current live-in synthetic gap rows after the transfer replay preserves restore-trampoline stack-register operations. The focused GPR/scratch and `EAC_VMTAIL_MEM=1` traces remain useful archived evidence for native tail mechanics, but they no longer feed active live-in comments into `vm_program_pseudocode_full.c` or `vm_recovered_source_bundle.c`. Consequently `vm_live_in_final_tail_site_probe.py`, `vm_synthetic_gap_live_in_reentry_probe.py`, and `vm_synthetic_gap_allstatic_reentry_probe.py` also emit 0 data rows in the current manifest. The remaining blocker is not live dispatch-slot GPR recovery; it is turning the symbolic slot expressions and dynamic/hidden-chain evidence for the 20 unresolved successor calls into recovered block starts or statically replayed hidden chains.

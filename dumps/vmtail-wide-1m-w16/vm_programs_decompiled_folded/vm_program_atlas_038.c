...
void vm_program_atlas_038_decompiled_folded(VMState *vm, uint64_t vm_ip) {
    while (1) {
        switch (vm_ip) {
    case 0x2bf97cu: {
    ...
    vm_ip += signed_vm_delta_u32(U32(vm->ip + 0x4));
        /* folded MBA successor: raw=0x2bfe6f, folded=-, entry=354, edge="decoded_control", mode=external_or_program_exit; "next = table[354], ip += 0x4c3" */
        next_entry = 354;
        vm_program_external_edge(vm, UINT64_C(0x2bfe6f), next_entry);
        (void)state0;
        (void)flags0;
        (void)byte0;
        return;

...
/* VM program atlas 049: 0x336dc8..0x33710f */

void vm_program_atlas_049_decompiled_folded(VMState *vm, uint64_t vm_ip) {
    while (1) {
        switch (vm_ip) {
    case 0x336dc8u: {
    ...
    next_entry = 299;
    vm_ip += 0x4;
        /* folded MBA successor: raw=0x33710f, folded=-, entry=299, edge="covered_synthetic_fallthrough", mode=external_or_program_exit; "next = 299@0xb6bba:1, ip += +0x4; dispatch_model=static_100; state=state_add_const" */
        next_entry = 299;
        vm_program_external_edge(vm, UINT64_C(0x33710f), next_entry);
        (void)state0;
        (void)flags0;
        (void)byte0;
        return;

...
void vm_program_atlas_064_decompiled_folded(VMState *vm, uint64_t vm_ip) {
    while (1) {
        switch (vm_ip) {
    case 0x3716c7u: {
    ...
    vm_ip += signed_vm_delta_u32(U32(vm->ip + 0x4));
        /* folded MBA successor: raw=0x37142c, folded=-, entry=354, edge="decoded_control", mode=external_or_program_exit; "next = table[354], ip -= 0x2e8" */
        next_entry = 354;
        vm_program_external_edge(vm, UINT64_C(0x37142c), next_entry);
        (void)state0;
        (void)flags0;
        (void)byte0;
        return;
    ...
    next_entry = (int)U32(vm->ip + 0x0);
    vm_ip += signed_vm_delta_u32(U32(vm->ip + 0x4));
        /* folded MBA successor: raw=0x3717c4, folded=-, entry=165, edge="decoded_control", mode=external_or_program_exit; "next = table[165], ip += 0x78" */
        next_entry = 165;
        vm_program_external_edge(vm, UINT64_C(0x3717c4), next_entry);
        (void)state0;
        (void)flags0;
        (void)byte0;
        return;

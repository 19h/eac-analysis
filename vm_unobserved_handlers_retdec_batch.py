...
def load_unobserved_ranges(semantics_path, table_path, batch_index, batch_size):
    ...
    for row in read_rows(table_path):
        ...
        if stop <= start:
            continue
        rows.append(row)
    rows.sort(key=lambda row: (int(row.get("span_bytes", "0")), int(row["entry"])))
    batch_start = batch_index * batch_size
    selected = rows[batch_start:batch_start + batch_size]
    ranges = [f"{row['target']}-{row['static_stop']}" for row in selected]
...
def main():
    parser = argparse.ArgumentParser(description="Emit targeted RetDec C for one batch of unobserved VM handler native ranges.")
    ...
    ranges, provenance, total_unobserved = load_unobserved_ranges(
        args.semantics,
        args.handler_table,
        args.batch_index,
        args.batch_size,
    ...
    if not ranges:
        raise SystemExit(f"no unobserved handler ranges found for batch {args.batch_index}")

    with tempfile.TemporaryDirectory(prefix=f"eacsym-retdec-unobserved-b{args.batch_index:02d}-") as tmpdir:
        out_path = Path(tmpdir) / "unobserved_handlers.c"
        cmd = [
            args.retdec,
            "--select-ranges",
            ",".join(ranges),
    ...
    print("/*")
    print(f" * Targeted RetDec C for unobserved VM handler native ranges, batch {args.batch_index}.")
    print(" *")
    print(" * These dispatch entries are statically inventoried but were not seen as")
    print(" * dispatch targets in the concrete traces. Batches are sorted by native")
    print(" * span size so each decompile stays small and reproducible.")
    print(" *")
    print(f" * Total unobserved handler ranges: {total_unobserved}")
    print(f" * Batch size: {args.batch_size}")
    print(" * Ranges:")
    for item, row in zip(ranges, provenance):
        print(
            f" *   {item} entry={row['entry']} span_bytes={row['span_bytes']} "
            f"insns={row['insns']} tail={row['tail_site']}"
        )
    print(" */")
    ...
    print("")
    print(functions)
    print(f"unobserved_handler_retdec_batch={args.batch_index}", file=sys.stderr)
    print(f"unobserved_handler_retdec_selected_ranges={len(provenance)}", file=sys.stderr)
